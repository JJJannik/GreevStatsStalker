package de.jjjannik.interactions.commands.minigames.skywars;
...
public class SkywarsTopCommand extends TopCommand {
    @Override
    public void execute(SlashCommandInteractionEvent evt) {
        handleTopCommand(evt, top -> {
            List<PvPPlayer> topStats = jga.getTopSkywars(top.amount(), top.offset());

            List<MessageEmbed> embeds = new ArrayList<>();

            EmbedBuilder builder = new EmbedBuilder().setColor(Color.GREEN).setTitle("Top " + top.amount() + " Skywars player starting at offset " + top.offset());
