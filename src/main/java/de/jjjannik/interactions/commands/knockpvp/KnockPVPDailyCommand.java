...
public class KnockPVPDailyCommand extends PlayerCommand {
    @Override
    public void execute(SlashCommandInteractionEvent evt) {
        handlePlayerCommand(evt, player -> {
            Instant now = Instant.now();
            Instant lastDay = now.minusSeconds(TimeUnit.DAYS.toSeconds(1));

            KillsDeathsPlayer stats = jga.getRollingKnockPvPPlayer(player.getUuid(), lastDay.getEpochSecond(), now.getEpochSecond());

            evt.replyEmbeds(new EmbedBuilder()
                            .setColor(Color.GREEN)
                            .setTitle("Daily KnockPVP stats of " + player.getName())
