...
public class KnockPVPTopCommand extends TopCommand {
    ...
    public void execute(SlashCommandInteractionEvent evt) {
        handleTopCommand(evt, top -> {
            List<KnockPVPPlayer> topStats = jga.getTopKnockPvP(top.amount(), top.offset());

            List<MessageEmbed> embeds = new ArrayList<>();

            EmbedBuilder builder = new EmbedBuilder().setColor(Color.GREEN).setTitle("Top " + top.amount() + " KnockPVP starting with #" + (top.offset()+1));
            EmbedBuilder builder1 = new EmbedBuilder().setColor(Color.GREEN);

            for (int i = 0; i < 50; i++) {
                if (topStats.size() == i) break;

                KnockPVPPlayer stats = topStats.get(i);
