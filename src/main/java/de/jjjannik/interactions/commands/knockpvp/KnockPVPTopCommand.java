...
public class KnockPVPTopCommand extends TopCommand {
    ...
    public void execute(SlashCommandInteractionEvent evt) {
        handleTopCommand(evt, top -> {
            List<KillsDeathsPlayer> topStats = jga.getTopKnockPvP(top.amount(), top.offset());

            List<MessageEmbed> embeds = new ArrayList<>();

            EmbedBuilder builder = new EmbedBuilder().setColor(Color.GREEN).setTitle("Top " + top.amount() + " KnockPVP starting at offset " + top.offset());
